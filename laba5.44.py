...
while True:
    ...
    console.print("[green]3[/green] - Відсортувати список за заданим атрибутом")
    ...
    if event.event_type == keyboard.KEY_DOWN:
        ...
        if choice == "1":
            ...
            try:
                ...
                console.print(
                    "[bold red]Помилка! Вік, фільми та статки мають бути цілими числами.[/bold red]"
                )
            ...
            if attr_event.event_type == keyboard.KEY_DOWN:
                ...
                if attr_choice in ["1", "2", "3", "4", "5"]:
                    ...
                    is_reverse = True if col_idx == 4 else False

                    database.sort(key=lambda x: x[col_idx], reverse=is_reverse)
                    console.print(
                        f"[bold green]Відсортовано за полем: {columns[col_idx]}[/bold green]"
                    )
                else:
            ...
            console.print(
                "\n[bold cyan]Оберіть атрибут для пошуку видалення:[/bold cyan]"
            )
            for i, col in enumerate(columns, 1):
                console.print(f"[green]{i}[/green] - {col}")
            ...
            console.print("\n[bold cyan]Оберіть атрибут для фільтрації:[/bold cyan]")
            for i, col in enumerate(columns, 1):
                console.print(f"[green]{i}[/green] - {col}")
